      ...
      <el-form :model="filters" class="filter-form">
        <div class="filter-grid">
          ...
          <el-form-item label="基金规模 ≥ (亿)">
            <el-input-number v-model="filters.minFundSize" :min="0" :max="500" :step="1" size="small" controls-position="right" />
          </el-form-item>
...
function calcFundMetrics(history) {
  ...
  // 使用累计净值计算年化收益（稳健型基金分红频繁，单位净值会失真）
  const valueForReturn = accNetValues.length >= 2 ? accNetValues : netValues
  const annualReturn3y = calcAnnualizedReturn(valueForReturn, 3)
  // 使用累计净值计算最大回撤
