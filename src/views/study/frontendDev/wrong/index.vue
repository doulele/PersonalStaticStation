<template>
  <div class="wrong-page">
    <div class="page-top">
      ...
      <span class="page-count">{{ list.length }} 道错题</span>
    </div>
    ...
    <div v-loading="loading" class="wrong-list">
      <QuestionCard v-for="q in list" :key="q.id" :question="q">
        <template #actions>
          ...
          <el-button type="success" @click="markMastered(q)">已掌握 · 移除</el-button>
        </template>
      </QuestionCard>

      <el-empty v-if="!loading && !list.length" description="太棒了，没有错题 🎉" />
    </div>
  </div>
</template>
...
import { getWrong } from '@/api/knowledgeGraph'
...
async function markMastered(q) {
  const res = await store.dispatch('frontendDev/removeWrong', q.id)
  if (res.success) {
    ElMessage.success('已移除，继续保持！')
...
@media (max-width: 768px) {
  .wrong-page { padding: 16px 12px 40px; }
  .page-top { gap: 10px; margin-bottom: 16px; }
  .page-title { font-size: 20px; }
}

@media (max-width: 480px) {
  .wrong-page { padding: 12px 8px 32px; }
}
</style>

<style lang="scss">
html.dark-mode .wrong-page {
